...
def segment_grain(path):
    ...
    # plt.figure(figsize=(10, 10))  #创建图形窗口
    ...
    for xml_name in img_total:
        ...
        df_list = []  # 保存单张图片中所有谷粒数据
        image = cv2.imread(os.path.join(images_xml_dir, filename))
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        predictor = SamPredictor(sam)
        predictor.set_image(image)
        folder_path = os.path.join(output_dir, xml_name)  # 为每张图片创建文件夹
        ...
        for i, mask in enumerate(masks):
            ...
            filenum = xml_name + '_' + str(i + 1)  # 为每颗谷粒创建文件夹
        ...
        # plt.yticks([])
        # 保存图像
        sam_path = os.path.join(output_dir, "SAM_images")
...
if __name__ == '__main__':
    image_path = "/root/autodl-tmp/test/"  # 图片及YOLO分割文件
