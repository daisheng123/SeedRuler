'''
rotation_base,just visualize images,big images,drow_one_box
no xml
add base excel
add per excel
'''
...
from zjf_tools.add_utils import move_files, txt_xml
from zjf_tools.zjf_eage_1 import count_pixel


def detect(file_path, save_img=False):
    out, source, weights, view_img, save_txt, imgsz, overlap = \
        increment_path(
            Path(opt.save_dir) / 'exp'), file_path, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.overlap
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        dict_list[nrows] = [osp.split(Path(p))[-1], w_means, h_means, total_numbers]
        ncols += 1
        nrows += 1
        per_dict_list = {}
