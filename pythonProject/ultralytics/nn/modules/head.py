...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "DetectAux", "OBBAux"
...
class DetectAux(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cva1 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4*self.reg_max, 1)) for x in ch[self.nl:]
        )
        self.cva2 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[self.nl:])
    ...
    def forward(self, x):
        ...
        if self.training:  # Training path
            for i in range(self.nl, 2*self.nl):
                x[i] = torch.cat((self.cva1[i-self.nl](x[i]), self.cva2[i-self.nl](x[i])), 1)
            return x[:self.nl]
