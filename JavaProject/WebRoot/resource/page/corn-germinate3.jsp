...
<body>
			...
			<form class="layui-form" action="" method="post"
				...
				<div class="layui-upload" style="display:flex;">
					...
					<div style="margin:0 10px; display:flex;">
						...
						<button type="button" lay-submit lay-filter="submit" class=" layui-btn layui-btn-normal" id="measureButton">Run</button>
						<div id="coords_div" style="padding-left:10px;display:none"class="layui-select-title"><input style="width:215px" id="coords" type="text" placeholder="Seed coordinates:" value="" readonly="" class="layui-input layui-unselect"></div>
						<button style="display:none"type="button" lay-submit lay-filter="submit" class=" layui-btn " id="calculate">calculate</button>
	...
	<div class="layuimini-main1" style="margin-top:-15px">
			...
			<div class="layui-row layui-col-space15">
				<div id="result" class="layui-col-xs12 layui-col-md4">
					<!-- <div id="echarts-records"
						style="background-color:#ffffff;min-height:270px;padding: 10px;padding-top:20px;">
						<img id="resultimage" style="max-width: 100%; max-height: 100%;"
							src="resource/images/N7-1-2A-1.jpg">
						<button type="button" lay-submit lay-filter="submit" class=" layui-btn" id="up">Up</button>
						<button type="button" lay-submit lay-filter="submit" class=" layui-btn layui-btn-normal" id="down">Down</button>
					</div> -->
					<div id="echarts-records"
						style="background-color:#ffffff;min-height:270px;padding: 10px;padding-top:20px; position: relative;">
						<img id="resultimage" style="max-width: 100%; max-height: 100%;"
							src="resource/images/N7-1-2A-1.jpg">
						 <button type="button" class="carousel-button" id="shang">&#8249;</button>
  						 <button type="button" class="carousel-button" id="xia">&#8250;</button>
					</div>
				</div>
				<div id="echarts" class="layui-col-xs12 layui-col-md8">
					<div id="echarts-pies"
						style="background-color:#ffffff;height:280px;padding: 10px;"></div>
				</div>
			</div>
			<!--  <div class="layui-row layui-col-space15">
				<div class="layui-col-xs12 layui-col-md6">
		...
		function changeImage(change,length,imagePath) 
		{
           ...
           lastIndex = imagePath[parseInt(change)%parseInt(length)].lastIndexOf('/');
		   imageIndex=parseInt(change)%parseInt(length)
           }
           else {
           image.src = imagePath[(parseInt(length)-parseInt(change))%parseInt(length)];
           lastIndex = imagePath[(parseInt(length)-parseInt(change))%parseInt(length)].lastIndexOf('/');
		   imageIndex=(parseInt(length)-parseInt(change))%parseInt(length)
           }
           document.getElementById("filename").textContent =imagePath[imageIndex].substring(lastIndex + 1);
			...
			*/
			...
			layui.use([ 'form', 'upload', 'layer', 'echarts' ], function() {
				...
				document.getElementById("measureButton").onclick = function() {
					...
					xhr.open("POST", "SAM/germinatemeasure_SAM", true);
					xhr.onreadystatechange = function() {
						if (xhr.readyState === 4 && xhr.status === 200) {
							...
							if (response.status == "success") {
								...
								console.log(response.imagePath[0])
								...
								//点击切换图片
								var up=document.getElementById("shang");
								var down=document.getElementById("xia");
								...
								document.getElementById("filename").textContent =response.imagePath[0].substring(response.imagePath[0].lastIndexOf('/') + 1);
							
								
								document.getElementById("coords_div").style.display = "block";
								...
								resultimage.addEventListener('click', function(event) {
									...
									var x =  Math.floor((event.clientX - rect.left)*(response.width/465.22));
									var y =  Math.floor((event.clientY - rect.top)*(response.height/261.99));
									coords.value = 'Seed coordinates:(' + x + ', ' + y + ')';
								});
				...
				document.getElementById("calculate").onclick = function() {
					...
					var coordinates = input.match(/\d+/g); // 提取字符串中的数字
					var pos_x = (coordinates[0]);
					var pos_y = (coordinates[1]);
					console.log(pos_x,pos_y,filename)
					// 创建一个表单数据对象
					var formData = new FormData();
					formData.append("imageSrc", imageSrc);
					formData.append("algorithm", algorithm);
					formData.append("filename", filename);
					formData.append("pos_x", pos_x);
					formData.append("pos_y", pos_y);
					
					// 发送POST请求给控制器
					var xhr = new XMLHttpRequest();
					xhr.open("POST", "SAM/SAM", true);
					xhr.onreadystatechange = function() {
						if (xhr.readyState === 4 && xhr.status === 200) {
							// 请求成功的处理逻辑
							var response = JSON.parse(xhr.responseText);
							...
							if (response.status == "success") {
								...
								document.getElementById("resultimage").src = response.imagePath[0];
								document.getElementById("result").style.display = "block";
								document.getElementById("echarts").className = "layui-col-xs12 layui-col-md8";
			...
			layui.use([ 'form', 'upload', 'layer' ], function() {
				...
				document.getElementById("batchInput").addEventListener("change", function(event) {
					...
					$.ajax({
						url : 'SAM/batchUpload', // 替换为您的后端接口地址
						type : 'POST',
						data : formData,
						processData : false,
						contentType : false,
